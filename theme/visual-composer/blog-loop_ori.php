...
if (!function_exists('mint_vc_blogloop_sc'))
{
	function mint_vc_blogloop_sc($atts, $content = null)
	{
		...
		{
			$inner_class = "blog-grid blog-inner-content";
			$ssa = ($fullwidth) ? "" : "max-width:700px;";

		}

			
				$grid_class = "col-sm-" . (12 / $cols);
								...
								{	
									while (have_posts())
									{
										...
										if ($blog_layout == "grid")
										{
											...
											{
												echo "</div>";
												$last_div_closed = true;
												#echo "<div class='clear'>&nbsp;</div>";
											}
											if ($wp_query->found_posts == $i && !$last_div_closed)
											{
												echo "</div>";
												#echo "<div class='clear'>&nbsp;</div>";
											}
