<style><?php global $of_options; 
		foreach ($of_options as $option)
		{
			if (array_key_exists('type', $option) && $option['type'] == "heading")
			{
				$header_class = str_replace(' ','',strtolower($option['name']));

				echo "#of_container #of-nav li.".$header_class." a { background-image:url('". get_template_directory_uri() . '/images/options/' . $header_class . '.png' . "'); }\n";
			}
		}
	?></style>
...
<div class="wrap" id="of_container">
	...
	<form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
		...
		<div class="save_bar"> 
			...
			<img style="display:none" src="<?php echo ADMIN_DIR; ?>assets/images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
			<button id ="of_save" type="button" class="button-primary"><?php _e('Save All Changes', 'optionsframework');?></button>			
