...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'webfont_typography':
					...
					if(isset($typography_stored['face'])) {
						...
						$faces = array( 'customfont' => 'Custom Font',
										'arial'=>'Arial',
										'verdana'=>'Verdana, Geneva',
										'trebuchet'=>'Trebuchet',
										'georgia' =>'Georgia',
						...
						if (class_exists('GoogleWebFonts'))
						{
				...
				case "messagebox":
					...
					$data[$value['id']] = (!isset($data[$value['id']])) ? $defaults[$value['id']] : $data[$value['id']];
					$info_text = $value['std'];
					$mbtype = (!isset($value['mbtype'])) ? "info" : $value['mbtype'];

					$output .= '<div class="of-alert of-alert-'.$mbtype.'">'.$info_text.'</div>';
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		if(!empty($upload)){
	    	...
	    	{
	    		$uploader .= '<img class="of-option-image ofaa" id="image_'.$id.'" src="'.$upload.'" alt="" />';
	    	} 
	    	$uploader .= '</a>';
			$uploader .= '</div>';
			}
	...
	public static function optionsframework_retina_uploader_function($id,$std,$mod){
	    ...
	    $uploader .= '<div class="clear"></div>' . "\n";
	   	...
	   	if (is_array($upload))
	   	{
			$ext = pathinfo($upload[0], PATHINFO_EXTENSION);
	   	}
	   	else
	   	{
	   		$ext = pathinfo($upload, PATHINFO_EXTENSION);
		...
		if(!empty($upload)){
	    	...
	    	if (in_array($ext, array('jpg', 'jpeg', 'png', 'gif') ) )
	    	{
	    		$uploader .= '<img class="of-option-image ofbb" id="image_'.$id.'" src="'.$upload.'" alt="" />';
	    	} 
	...
	public static function optionsframework_media_uploader_function($id,$std,$int,$mod){
		...
		if(!empty($upload)){	
	    	...
	    	if (in_array($ext, array('jpg', 'jpeg', 'png', 'gif') ) )
	    	{
	    		$uploader .= '<img class="of-option-image ofcc" id="image_'.$id.'" src="'.$upload.'" alt="" />';
	    	} 
	    	$uploader .= '</a>';			
			}
		$uploader .= '</div>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		if(!empty($val['url'])){
	    	...
	    	$slider .= '<img class="of-option-image ofdd" id="image_'.$id.'_'.$order .'" src="'.$val['url'].'" alt="" />';
	    	$slider .= '</a>';
